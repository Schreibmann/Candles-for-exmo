...
function makeChart(pair, limit, period) { // limit устанавливает количество возвращаемых сделок от 1 то 10000, period - период закрытия свечей в минутах
           ...
           for (let deal of deals) { 
             if (deal.date < tickDivider) { // Если сделка в пределах периода, записываем ее в тик
               tick.push(deal); 
                 ...
                 let candle = {};
                 let tickPrices = tick.map(deal => deal.price);           
                 let candlePrices = [parseFloat(tick[0].price), 
                                     Math.max(...tickPrices), 
                                     Math.min(...tickPrices), 
